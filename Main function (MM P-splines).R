...
m.pen.sp <- function(x, y, norder = 4, nbasis = NULL,  k = 4.685, q  = 2, n.se = 1){
  ...
  GCV <- function(lambda){
    ...
    # GCV.scores <- mean(Mpsi(lred/mad(lred), deriv = -1, psi = "bisquare", cc = k))
    # GCV.scores <- median(lred^2)
  ...
  # sdw <- scaleTau2(Mpsi(lred/mad(lred), deriv = -1, psi = "bisquare", cc = k) - GCV(lambda1), c2 = 5)/sqrt(n)
